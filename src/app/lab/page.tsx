/**
 * ============================================================
 * 実験用の画面（位置情報・地図・通り道のスポット）— /lab
 * ============================================================
 *
 * ■ このファイルは何？
 *   Next.js（App Router）では、ファイルを置いた場所がそのままURLになる。
 *     src/app/page.tsx        → /       （アプリ本体。クエストの画面）
 *     src/app/lab/page.tsx    → /lab    （この画面）
 *
 * ■ この画面の役割
 *   ボタンを押すと現在地を取得し、地図・マーカー・誤差の円・数値を表示する。
 *   移動すると自動で追従し、停止ボタンでGPSを止められる。
 *
 *   **遊びの本編ではない。** 仕様書§2.4「移動中の表示」（任意機能）と、
 *   その材料になる部品を、単体で動かして確かめるための画面として残してある。
 *   本編に組み込むときは、ここのコードを移していく。
 *
...
export default function Home() {
  ...
  return (
    ...
    <main className="mx-auto flex min-h-screen max-w-md flex-col gap-4 p-5">
      ...
      {/* ------------------------------------------------------------
          選んだスポットの情報（仕様書§2.4③「スポットまでのルート表示」）
          ------------------------------------------------------------
          これは通り道の寄り道先であって、クエストの行き先ではない。
          クエストの行き先は移動カードが座標を持たないので存在せず、
          ルートも原理的に出せない（仕様書§2.4④）。
          寄り道先はクエストと無関係なので、名前もルートも隠す必要がない。 */}
