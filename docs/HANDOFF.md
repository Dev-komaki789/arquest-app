...
> **2026-08-03 に大きな仕様転換があった。§1.5 を先に読むこと。**
> コンセプト（§1）とお題の構造が変わっている。実装済みのコードはほぼ全部生きるが、
> ⑧以降の設計は作り直しになった（未着手なので損失はない）。
...
| ⑤ | ルートを取得し地図に線を描く | ✅ 完了（OSRM→Valhalla。**ただし本編では使わないことになった**。§1.5） |
| ⑥ | `watchPosition` で現在地を継続取得し追従 | ✅ 完了 |
| ⑦ | Haversineで距離を計算 | ✅ 完了 |
| ⑧ | PostGISを有効化し到達判定のRPCを作る | ⬜ **次はここから**（§6参照） |
| ⑨ | 到達時にRPCを呼びサーバー側で最終確認 | ⬜ 未着手 |
| ⑩ | 到着後に行動カードを抽選し行動確認画面を出す | ⬜ 未着手 |

仕様書§9の最小構成のうち「現在地マップ／実在スポットの取得」まで到達。
残りは「到達判定（GPS・PostGIS）／行動カードと自己申告／相棒（静止画）」。

---

## 1.5 2026-08-03 の仕様転換（重要）

1日の議論で**10個の決定**が出た。仕様書には反映済み。ここには**判断の理由**を残す。

### なぜ変わったか

きっかけは2つ。**さんぽ神**（ドロッセルマイヤー商會のカードゲーム。カードを引いて
「お告げ」として散歩の指示を受け取る）を参照先にしたこと。
そして**実際のプレイ動画を見て、カードの構造を勘違いしていたと分かった**こと。

### 決定一覧

| # | 決定 | 理由 |
|---|---|---|
| 1 | **§1を「ただ歩くという苦行を、遊びにする」に転換** | 画面・アプリ名・デザインは最初から遊びの語彙だった。§1だけが「課題解決」で取り残されていた。プレゼンが重くならない／対象が広がる |
| 2 | **お題＝「移動カード × 行動カード」** | 実プレイを見ると、前半のカードは**場所ではなく移動のしかた**だった（「適当に30分歩いて」「降りたことないバス停で降りて」） |
| 3 | **場所のカテゴリ（4分類）を廃止** | 畳むと情報が失われる。移動カードは「公園」「神社」と**具体的に名指しする**ので粒度が足りない。OSMタグをそのまま持つ |
| 4 | **ハチャメチャを許容する** | 止めるのは「実行不能」と「迷惑」だけ。「コンビニで空を見上げる」は**むしろ狙い** |
| 5 | **行動カードは観察型を主軸に** | 「一番古そうなものを探す」は**どこでも成立するが答えは場所ごとに違う**。指示は汎用、答えは場所固有 |
| 6 | **骨格 × 変数で作る** | 固有名詞（芸能人・アニメ）は賞味期限があるので変数側に隔離し、差し替え可能にする |
| 7 | **抽選は「チェック → 抽選」の順** | 逆にすると外れるたびに引き直しになり、郊外ほど遅くなる。候補を絞ってから引けば必ず1回で決まる |
| 8 | **抽選は2回に分ける**（移動＝出発前／行動＝到着後） | 出発前に両方引くと驚きが消える。**伏せるのではなく、まだ存在しない** |
| 9 | **帰り道は設計しない** | 疲れ切っている可能性が高く、お題を出すと遊びではなく**義務**になる |
| 10 | **地図塗りは実地図方式** | 要件は「行ったスポットが見た目で分かればOK」。抽象グリッドと達成率は不要 |
...
|---|---|
| ①②⑥ 現在地・地図・追従 | **影響なし** |
| ③④ Overpass取得＋キャッシュ | **重要度が上がる。**移動カードの実行可能性判定に必須 |
| ⑦ Haversine距離 | **中心的になる。**「100m以内に入ったか」を見張る発火装置 |
| [useWalkTrail.ts](../src/hooks/useWalkTrail.ts) の距離計測 | **表示用の統計から、クリア判定の心臓部に昇格**（「30分歩いて」の判定） |
| ⑤ Valhalla | **本編では出番なし。**行きは道順を出すと台無し、帰りは設計しないため。§4-1の実測記録は残る |
| ⑧⑨⑩ | 設計が変わったが**未着手なので損失ゼロ** |

**⑧が作りやすくなった。** 「到着してからクエストを作る」設計なので、
§6-1で指摘した「`quests` を作るには `users` と `quest_templates` が要る」という
芋づるが緩む（事前生成のバッチが不要になるため）。

### 未解決だった問題が、まとめて消えた

- **候補の枯渇**（§6-3）→ 型A「30分歩いて」が常に候補に残るので構造的に起きない
- **カテゴリの偏り**（§6-4。nature が120件中3件）→ しぜんは**場所ではなく行動側**にあるので消滅
- **対人度★の段階数が3者で食い違う問題** → 対人を伴う行動を作らないので概念ごと不要
- **「お題を伏せる」の解釈違い** → 伏せるのではなく、まだ存在しないので自然に解決
- **営業時間の考慮**（夜は店が閉まっている）→ 飲食・購買のお題を作らないので不要

### 技術力をどこで見せるか

「ランダムにお題を出すだけなら技術のアピールが消える」という懸念に対する結論。

**行動カードはどこでも成立してよい。技術はそこに置かない。**
位置情報が本質的に必要なのは**移動カード側**。技術は次の2か所に集約する。

1. **移動指示の生成** — 未訪問スポットの空間検索・実行可能性の判定
2. **到達と距離の判定** — クライアント概算＋サーバーPostGISの二段構え

どちらも**ゲームが成立するために要るもの**なので、取ってつけた感が出ない。
**これ以外に技術を足すと「アピールのために付けた」と見透かされる。**

そして最大のアピールは機能の量ではなく、**下の §4 の判断記録**である。
「動くものを作った」ではなく「**エラーが出ない不具合を実測で見つけ、
トレードオフを数字で判断した**」という記録になっている。機能を減らしても減らない。
...
> ### ⚠ この2つの関数のSQL定義が、リポジトリのどこにも残っていない
>
> テーブル定義（§5-3）はコピペできる形で残っているのに、**関数だけが穴**になっている。
> 関数は**Supabaseのデータベースの中にしか存在しない。**
> `route.ts` は名前で呼んでいるだけで、中身を知らない。
>
> **困る場面**
> - 訓練校用に別のSupabaseプロジェクトを作る場合 → **作れない**。④のキャッシュが全滅する
> - Supabaseプロジェクトを失った場合 → 復旧できない
> - ポートフォリオとして「環境を再現できない」状態でもある
>
> **対処（5分）** SQL Editor で次を実行し、出てきた定義を `docs/schema.sql` に保存してコミットする。
> §5-3 のテーブル定義も一緒にまとめておくと、以後どこでも環境を再現できる。
>
> ```sql
...
> ```
>
> 管理画面から見るなら **Database → Functions** にも一覧がある。
...
| | 内容 | なぜ先か |
|---|---|---|
| **A** | **RPC定義を `docs/schema.sql` に保存**（§5-4の警告） | 5分で終わる。環境を再現できない状態を放置しない |
| **B** | **移動カード・行動カードを実際に書き出す**（骨格20 × 変数5 が目標） | 仕組みより先に**お題の手触り**が決まらないと、テーブル設計の良し悪しを判断できない |
| **C** | **`pois` に `place_type` を足して取り直す**（仕様書§6-2） | いま約120件しかないので**消して取り直すのが一番安い**。溜まるとOverpassの回数制限に当たる |

**Bが一番大事。** 仕様書§6-1のカード関連テーブルは素案のままで、
実物のカードを20個ほど並べてみないと、変数の切り方も型の分け方も決められない。

### 6-1. 仕様書§8の順序には、まだ飛びがある

⑧の到達判定RPCは `quests` テーブルを必要とし、`quests` は `users` を参照する。
仕様書§8の順序は、**ログイン機能（画面1）と `users` を飛ばしている。**
そのまま着手すると、先に作るものが芋づる式に出てくる。

※ 2026-08-03 の変更で、`quest_templates` への依存は消えた（`move_cards` / `action_cards` に置き換え）。
また「到着してからクエストを作る」設計になったので、**事前生成のバッチが不要**になり、飛びは以前より小さい。

### 6-2. 進め方は2つ

| | 内容 |
|---|---|
| 土台を先に作る | ログイン（画面1）→ `users` → カード各表 → `quests` → ⑧。正攻法だが着手までが長い |
...
**役割が変わった（2026-08-03）。** `visit_logs` は統計用のログではなく、
**お題そのものの材料**になった。移動カード型B「**行ったことない**公園まで歩いて」の
「行ったことない」を判定するのが `visit_logs` である。

そして**これはカードゲームには原理的にできない。**さんぽ神はプレイヤーがどこに行ったことがあるかを
知らないので、記憶に頼るしかない。アプリは本当に知っている。**ここがアプリである必然性。**

**旧「未解決の論点: 候補が枯渇する」→ 解決済み。**
以前は「近所のスポットは有限なので、除外し続けるといつかゼロになる」ことを懸念していた。
移動カードに**型A（30分適当に歩いて）が常に候補として残る**ようになったため、
「引けるカードが1枚もない」状態は構造的に起きなくなった。

型Bだけが枯れたときの緩和策は、必要になってから決めればよい（以前の検討）。
...
都市部では**「しぜん」のお題がほぼ出せない**——というのが、当時の問題意識だった。

**これも解決済み（2026-08-03）。**「しぜん」は**場所ではなく行動側**にあった。
「空を見上げて雲の形を言葉にする」「風の向きを手のひらで確かめる」「植え込みで一番元気な葉を探す」は、
**コンビニの前でも成立する。**カテゴリを行動カードに移したことで、この偏りは問題でなくなった。

観察としては引き続き有効なので数字は残しておく。
`place_type` を保存するようになれば、より細かい分布が見えるはず。
...
**2026-08-03 に反映済み。** §1.5 の決定10件をすべて仕様書に書き込んだ。主な変更箇所：

- **§1** コンセプトを全面書き換え（＋転換の理由と、意識して残すことを明記）
- **§2.2** 「お題の構造 — 移動カード × 行動カード」に全面書き換え
- **§2.4** 「であい系だけ行動確認」→「全クエストで行動確認」
- **§2.5** 相棒＝お告げの語り手・ゲームマスター
- **§2.6** 地図塗りを実地図方式に
- **§2.8 / §2.9**（新設）遊ぶ範囲の設定／遊びの流れ
- **§4-1**（新設）プロトタイプへのデザイン指摘6件
- **§5** Valhallaは本編で使わない旨／技術力をどこで見せるかの整理
- **§6** `pois.place_type` へ変更、`quest_templates` 廃止、カード関連テーブルの素案
- **§7** RPCを `draw_move_quest` / `complete_arrival` の2つに再設計
- **§8** ⑧の前提（`users` を飛ばしている件）を注釈として明記
- **§11** 却下・決定の記録を7件追加

### まだ仕様書に反映していないこと

- **移動カード・行動カードの実物**（§6-0 の B）。書き出してから §6-1 の素案を確定させる。
- **§4-1 のデザイン指摘6件**は「未決」のまま記録しただけ。採否の判断が要る。
...
- **README には §1.5 の「技術をどこに置くか」の整理も書く。**
  「機能を減らしたのに技術力が落ちていない」ことを、判断として説明できる形にする
