...
> 次にやるのは §6。設計の背景を知りたいときだけ §1.5 を読めばよい。
...
/lab         位置情報と地図の実験用（本編ではない）
```

### まだ無い画面（下のタブに薄く出してある）

相棒・マップ（地図塗り）・設定・稼働時間設定・ログイン（匿名のみ）。

### 位置情報を使う場所（ここが設計の要）

| 場面 | 使うか |
|---|---|
| 移動カードを引く・移動中 | **使わない**（行き先の正解が存在しないため） |
| 移動中の表示をオンにしたとき | スポット探しにだけ使う。**軌跡は残さない** |
| 行動カードを引いた後 | 使う。ここから軌跡と距離を記録する |

## 1.6 見た目の決まりごと（2026-08-16 時点）

**モックアップは [screen-images/](../screen-images/) が最新。** 12枚あり、いまの画面はこれに準拠している。
（`screen-shot/` と `画面イメージ.png` は古い版。リポジトリには入れていない）

- **配色は緑**。ボタン・見出し・帯が緑、地は淡い緑、カードは白。
  ゴールドと紺は相棒の絵と地図でだけ残っている
...
```
src/app/page.tsx                本編（ホーム／移動カード／移動中／行動カード／達成）
src/app/diary/page.tsx          日記帳（一覧）
src/app/diary/[id]/page.tsx     日記の詳細（編集・削除）
src/app/lab/page.tsx            位置情報と地図の実験用（本編ではない）
src/app/manifest.ts             PWAの設定
src/components/CardFlip.tsx     カードをめくる
src/components/Companion.tsx    相棒スラりん（Figmaの絵3枚を出し分け）
src/components/Confetti.tsx     達成の紙吹雪
src/components/WalkScene.tsx    移動中に流れる景色
src/components/WalkingInfoPanel.tsx / DetourRoute.tsx  通り道のスポットと道順
src/components/DiaryComposer.tsx  日記を書く（写真はEXIFを消してから保存）
src/components/TabBar.tsx       下のタブ
src/lib/quest.ts                カードの抽選とクエストの読み書き
src/lib/diary.ts                日記（EXIF削除・非公開バケット・期限つきURL）
src/lib/profile.ts              ホームに出す記録
src/lib/settings.ts             移動中の表示のオンオフ
src/lib/supabase-browser.ts     画面からの接続と匿名ログイン
src/hooks/useQuest.ts / useQuestTrail.ts / useWalkingInfo.ts
...
### 6-0. まず実機で歩いて確かめる（2026-08-16 朝の予定・未実施）

作ったものは動いているが、**外で歩いた確認をまだしていない。**
次のセッションは、この結果を聞くところから始めるとよい。

| 見るところ | 気にする点 |
|---|---|
| ホーム画面に追加 → アイコンから開く | アドレス欄が消えるか |
| カードをめくる | 揺れ・めくり・横長表示 |
| 移動中の表示をオン → 300m歩く | スポットが出るか。ルートが出るか |
| 着いた → お題 → できた | 紙吹雪。EXPと距離が入るか |
| 写真つきで日記を1件 | **保存後に `node scripts/check-exif.mjs --list` → `--storage <場所>` で、GPSが消えているか実物で確認する** |
| 画面を消して歩く | **距離が止まる可能性が高い。**Androidはページを凍らせる。止まったら対策が要る |
...
| 画面 | 必要なもの | 備考 |
|---|---|---|
| 設定 | 移動中の表示・通知 | いまは移動中の画面に切り替えが埋まっている |
| 稼働時間 | 平日／休日の時間帯 | 通知の前提。`user_settings` に列はある |
| マップ（地図塗り） | `visited_cells` を地図に描く | **データは今日から溜まっている**ので、作れば過去ぶんも出る |
...
仕様書§2.9で**距離を報酬に変える唯一の経路**とされているが、まだ実装していない。
着せ替えとセットなので、優先度は通知より低い。

### 6-4. 通り道のスポットで残っている作業

- 取得済みの `pois` から先に出し、足りない分だけ問い合わせる（いまは毎回問い合わせる）
- 病院・学校・警察・墓地・斎場を除外する（[CARDS.md](./CARDS.md) §1 と同じ理由）
...
## 8. 今日つまずいたところ（同じ穴に落ちないために）

2026-08-15〜16 に実際に踏んだもの。コード側にもコメントを残してある。
...
- 画面の並びと配色は [screen-images/](../screen-images/) が最新で、仕様書§4はまだ古い記述
